...
\begin{figure}[htbp!]
	\begin{subfigure}{\textwidth}
		...
		\caption{ Grid search with 9 (left), 25 (center), and 49 (right) grid points. The grid is homogeneous and the function values are indicated with the colorbar on the side. The best configuration is indicated with a white cross on the grid point and achieves a model performance of \textbf{0.14393} (left), \textbf{0.14080} (center), and \textbf{0.14393} (right) for mean absolute percentage with 2-fold cross validation. }
	...
	\begin{subfigure}{\textwidth}
		...
		\caption{ Sparse grid optimization with 7 (left), 27 (center), and 47 (right) grid points. The sparse grid is adaptive and the function values are indicated with the colorbar on the side. The best configuration is indicated with a white cross on the grid point and achieves a model performance of \textbf{0.14393} (left), \textbf{0.13739} (center), and \textbf{0.13533} (right) for mean absolute percentage with 2-fold cross validation. }
		\label{fig:first_comparison_sparse_optimization}
	\end{subfigure}
	
	\caption{ Comparison of the grids generated for grid search (see Figure \ref{fig:first_comparison_grid_search}) and the sparse grid optimization (see Figure \ref{fig:first_comparison_sparse_optimization}) for the house\_sales dataset. }
...
\subsection{3 and 5-dimensional Experiments of Regression with Small Neural Network}
...
\begin{figure}[htbp!]
	...
	\begin{axis}[
		...
		\legend{Grid search, random search, bayesian, sparse}
