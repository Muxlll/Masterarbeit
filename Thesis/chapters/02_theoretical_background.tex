...
Depending on whether the optimization algorithm uses the gradient or not, it is called a gradient-based method or gradient-free method, respectively. In the following, algorithms of both types are presented \cite{b_splines}.
...
As well as the previous described sparse grid optimization, an adapted random search can also be used for finding the minimum of arbitrary functions. The so-called adaptive random search was also already used in different settings in multiple variations. \newline 

One example is presented in \cite{hamzaccebi2006heuristic}. The authors introduce Adaptive Random Search Technique (ARSET) and compare its performance with other optimization techniques. The algorithm dynamically adapts the search space depending on the current and new function value, as well as the one two epochs before. The authors describe that with their algorithm, not only local optima but also the global optimum can be found. Benchmark tests are presented that show that other techniques like David–Fletcher Method \cite{schilling1999applied} are outperformed. Note that the authors used up to 50000 epochs to find the best solution. This number is way too high for hyperparameter optimization because of the long model training and evaluation. \newline 

The authors of \cite{MASRI1980353} also introduce an alternative approach of making the random search adaptive. The search space in each iteration is adapted by changing the standard deviation. For the exact algorithm, refer to \cite{MASRI1980353}. The authors present tests that illustrate that the adaptive approach accelerates the convergence compared to the standard random search optimization. \newline 

In the context of hyperparameter optimization where neural networks are initialized with random weights, the evaluation with the same configuration might lead to stochastic results. This can be prevented by e.g. setting the seeds of random functions. However, the authors of \cite{random_search_2} present three different adaptive random search approaches for such stochastic optimization problems. The first one is adaptive search with resampling (ASR) and it reduces the noise of the objective function through resampling of already evaluated points. The other two methods, deterministic and stochastic shrinking ball (DSB, SSB), reduce this noise by averaging the function values inside the ball. The authors show that ASR is a very promising algorithm for situations where normal random search has problems identifying good solutions. \newline

\cite{schumer1968adaptive}: adaptive step size random search \newline 

\cite{kelahan1978application}: application of random search on discrete variables -> categorical or integer variables \newline 

\cite{tang1994adaptive}: partition search space into regions and refine 
